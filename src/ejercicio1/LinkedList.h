...
class LinkedList {
    ...
    public:
        ...
        void reverse() {
            if (head == nullptr) {
                return; // No hacer nada cuando la lista esté vacía
            }

            tail = head; // La cabeza actual es la nueva cola
            
            Node *prev = NULL; // Almacena el nodo anterior
            Node *current = NULL; // Recorre la lista
            Node *next = NULL; // Almacena el siguiente nodo antes de modificaciones
            ...
            head = prev; // El último nodo procesado se convierte en la nueva cabeza
        }
