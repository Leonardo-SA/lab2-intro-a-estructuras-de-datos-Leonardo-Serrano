...
class LinkedList {
    ...
    public:
        ...
        void removeDuplicates() {
            Node* current = head;
            while (current != nullptr) { // Se recorre cada nodo
                Node* runner = current;
                while (runner->next != nullptr) { 
                    if (runner->next->value == current->value) { // Si hay un duplicado
                        Node* temp = runner->next;
                        runner->next = runner->next->next;
                        delete temp; // Se elimina el nodo duplicado
                        length--;
                    } else {
                        runner = runner->next; // Se avanza al siguiente nodo
                    }
                }
