...
class DoublyLinkedList {
    ...
    public:
        ...
        void printList() {
            Node* temp = head;
            while (temp != nullptr) {
                cout << temp->value << " ";  // Imprime el valor del nodo actual
        ...
        bool isPalindrome() {
            ...
            while (left!=right && left->prev!=right) {
                ...
                // Si no coinciden los valores a la izquierda y a la derecha, retornar falso
                if (left->value != right->value)
                    return false;
                    
                // Se trasladan los punteros hacia la siguiente posición hasta toparse
                left = left->next;
                right = right->prev;
